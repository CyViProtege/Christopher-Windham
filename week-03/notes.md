...
```
A file is a single item while a folder is a directory/container that holds files.
```

**How does a Windows-style file path differ from a Linux-style file path?**

```
It consist of drive letters while linux style paths are more user based.
```

**One command from this week you feel most confident with — and why?**

```
cd. Navigating directories and finding files is the most essential skill using for cybersecurity so you have to use this command more than any other.
```
