...
```
Maintenance Note - Conveyor belt 3 serviced, next check up due in 90 days.
```
...
```
Maintenance Note - Conveyor belt serviced, next check due in 90 days.
```
...
```
cd sorted-findings, ls
```

Output:

```
maintenance-notes.txt shift-log.txt supply-inventory.txt
```
...
```
If I ever when to a wrong folder while on my scavenger hunt I would either input cd .. so I can return to the directory prior or cd ~ so I can return to the home folder and begin my dive into the directory that I hadn't checked yet.
```
...
```
The file that took the longest was the supply-inventory.txt. I presumed that some of the directories wouldn't have the files I was looking for because I explored them on a prior lab and through process of eliminated I discovered that I should have looked in those folders to begin with and went against my gut instinct.
```
...
```
Through repetition and practice I became extremely comfortable navigating these folders and making mistakes. I knew that I couldn't break anything and because of my detailed note taking I was very familiar with the inputs that I needed to use to complete the task at hand.
```
...
```
I'm looking forward to deepening my understand of chmod and permissions to see how it applies to real world scenarios. It was thrown into a few labs and I had no idea what I was doing. I've played with virtual machines in the past as well so a refresher course and strengthening my fundamentals with new task is exciting.
```
