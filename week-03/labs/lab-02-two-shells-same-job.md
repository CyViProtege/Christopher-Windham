...
```
One of the biggest differences I've noticed between Bash and PowerShell is how it presents the contents when you use the command ls/Dir. Bash will present a one sentence line of all the files in that direction while PowerShell will give you a more detailed listing of the files and associated drive letters.
```
...
```
When I was navigating between the Bash and PowerShell terminals the file system were identical in a few way. Even though the prompts slightly differed I still saw a README.md, intake, logs, and maintenance. Whenever I used ls in the logs file I would see the same shift-logs.txt as in most of the logs.
```
...
```
New-Item Get-Help Type. When it comes to Bash the commands are simpler and it feels as if I'm writing short-hand or abbreviated. PowerShell however isn't as forgiving and grammatical errors like punctuation can invalidate your code entirely. 
```
...
```
Understanding how case-sensitive PowerShell is makes it a lot easier to navigate. It does make me favor Linux systems a little more because the inputs and everything about the system all-together feels a lot easier to navigate. I often find myself using the Linux commands more than PowerShell now that I have a better understanding of which codes work with both terminals.
```
