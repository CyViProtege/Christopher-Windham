...
```
README.md intake logs maintenance & home/morgan
```
...
```
grep searches inside files for a pattern - useful when a folder holds many files
```
...
```
It's written linux style. Generally commands in Windows are case sensitive and have a (-) whereas Linux doesn't focus on case sensitivity as much/
```
...
```
A good rule of thumb is to verify your location and the files that are in the folder so that you're not removing or deleting anything you shouldn't. There is no recycle bin to save you from mistakes.
```
...
```
As a tech professional we're going to run into a lot of scenarios and explore commands as a part of our daily life. We won't always have notes and things to look back on from training so the man/Get-Help command takes the guess work out of it and can save a professional a lot of headache.
```
...
```
I was surprised at how complex some of these commands can be. It probably will get easier with time and muscle memory but I was thoroughly stumped on some of these labs. Navigating folders and peaking into files itself is a pretty simple complex, but overall I'm really looking forward to putting everything together and seeing how it applies to real world examples.
```
